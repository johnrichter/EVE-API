...
-(instancetype)init;
{
   ...
   if (self)
   {
      ...
      [self.url appendString:@"map/Sovereignty.xml.aspx"];
      
      self.cakAccessMask = @0;
      self.cacheStyle = kModifiedShortCache;
      
      // Built Object Properties
      self.systemSovereignties = @[];
...
-(void)configureObjectDescriptors
{
   ...
   XKObjectDescriptor *systemSovereignty = [[EVESolarSystemSovereignty new] objectDescriptor];
   [systemSovereignty setElementName:@"row"];
   ...
   [self.objectDescriptors addObjectsFromArray:@[systemSovereignty, dataTime]];
}
                                      ...
                                      Error:(NSError *)error
{
   ...
   {
      if (NSClassFromString(objectClass) == [EVESolarSystemSovereignty class])
      {
         NSMutableArray *systemSovereigntiesList = [NSMutableArray new];
         for (EVESolarSystemSovereignty *systemSov in builtObjects[objectClass])
         {
            [systemSovereigntiesList addObject:systemSov];
         }
         self.systemSovereignties = [NSArray arrayWithArray:systemSovereigntiesList];
      }
...
-(NSString *)description
{
   ...
   for (EVESolarSystemSovereignty *systemSov in self.systemSovereignties)
   {
      [str appendFormat:@"%@\n", systemSov];
   }
